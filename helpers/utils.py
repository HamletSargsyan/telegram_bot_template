...
def log(log_text: str, log_level: str, record: logging.LogRecord) -> None:
    ...
    log_template = (f'<b>{emoji_dict.get(log_level.lower(), "")} {log_level.upper()}</b>\n\n' 
                   f'{current_time}\n\n'
    ...
    for text in split_string(log_text, 3000):
        try:
            antiflood(bot.send_message, log_chat_id, log_template.format(text=escape(text)), message_thread_id=log_thread_id)
